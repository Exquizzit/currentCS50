...
def newItem(request):
    if request.method == "POST":
        ...
        itemImage = request.POST["image"]
        itemDescription = request.POST["description"]

        if not int(itemPrice) > 0:
            return render(request, "auctions/new_item.html", {
                "error": "Price has to be at least more than 0$"
            })

        item = Listing(name=itemName, image=itemImage, price=itemPrice, desc=itemDescription)
        item.save()
